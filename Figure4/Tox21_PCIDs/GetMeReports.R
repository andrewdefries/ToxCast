...
drugTargets <- activeTargets(pubChemDatabase, query[a])
...
############
write.table(drugTargets, file=paste("PCID_", query[a], "_drugTargets", sep=""), col.names=F, row.names=F, quote=F)
write.table(unique(drugTargetSpecies[,1]), file=paste("PCID_", query[a], "_drugTargetSpecies", sep=""), col.names=F, row.names=F, quote=F)
write.table(targetSequences, file=paste("PCID_", query[a], "_drugTargetSequences", sep=""), col.names=F, row.names=F, quote=F)
############
}
############
a<-1#:length(query)
